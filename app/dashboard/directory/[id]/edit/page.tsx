...
export default function EditDirectoryListingPage({ params }: { params: { id: string } }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    async function fetchListing() {
      try {
        const res = await fetch(`/api/directory/listings/${params.id}`, { cache: 'no-store' });
        ...
        setFormData({
          ...
          image_urls: Array.isArray(json.image_urls) ? json.image_urls.join(', ') : '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user) {
      router.push(`/login?redirectTo=/dashboard/directory/${params.id}/edit`);
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`/api/directory/listings/${params.id}`, {
        method: 'PUT',
  ...
  return (
    <div style={{ minHeight: '100vh', paddingTop: 40, paddingBottom: 40 }}>
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '0 20px' }}>
        ...
        <h1 style={{ marginBottom: 20 }}>Edit Business Listing</h1>
