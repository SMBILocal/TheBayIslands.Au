...
export default function FavoritesDashboardPage() {
  ...
  useEffect(() => {
    ...
    if (!user) {
      router.push('/login?redirectTo=/dashboard/favorites');
    }
  ...
  return (
    <div style={{ minHeight: '100vh', paddingTop: '40px', paddingBottom: '40px' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '0 20px' }}>
        ...
        <h1 style={{ marginBottom: '8px' }}>Saved Favorites</h1>
        <p style={{ color: '#666' }}>Your saved items will appear here.</p>
        <div style={{ marginTop: '20px', padding: '16px', background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '8px' }}>
          <p style={{ margin: 0, color: '#6b7280' }}>No favorites yet. Save listings across the site to see them here.</p>
        </div>
      </div>
    </div>
  );
